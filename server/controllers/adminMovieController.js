...
export const getAllMovies = async (req, res) => {
  try {
    ...
    const movies = await Movie.find()
      .populate({ path: "addedByAdmin", select: "name email", strictPopulate: false })
...
export default {
  ...
  fetchNowPlayingFromTMDB: (req, res) => res.status(410).json({ success: false, message: "TMDB integration disabled." }),
  importMovieFromTMDB: (req, res) => res.status(410).json({ success: false, message: "TMDB integration disabled." }),
};
