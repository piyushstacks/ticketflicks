...
// Import the dummy data from assets
...
}, { collection: 'movie_tbls' }));

async function updateMoviesWithCasts() {
    try {
        ...
        // Update all movies with cast data
        const result = await Movie.updateMany(
            {}, // Update all movies
            { $set: { casts: dummyCastsData } }
        );

        console.log(`Updated ${result.modifiedCount} movies with cast data`);
        console.log('Successfully updated movies with cast data');

    } catch (error) {
        console.error('Error updating movies with cast data:', error);
    } finally {
