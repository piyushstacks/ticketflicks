...
const DB_NAME = "ticketflicks"; // Target the correct database
  ...
  for (const oldMovie of oldMovies) {
    try {
      ...
      await NewMovie.create({
        ...
        release_date: oldMovie.release_date || new Date("2024-01-01"),
        description: oldMovie.overview || `Watch ${oldMovie.title || "this movie"} at TicketFlicks.`,
...
const runMigration = async () => {
  try {
    log("🚀 Starting Movie-only migration to ticketflicks DB...");
    ...
    log(`✅ Connected to MongoDB (DB: ${mongoose.connection.db.databaseName})`);
